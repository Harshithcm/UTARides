...
import com.se.view.SelectedUserDetails;
...
public class LoadAvailableList extends Activity {
	JSONArray jsonArray;
	ListView listView;
	ListAdapter adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		rideData.setLocation(locationSearch);
		rideData.setDay(dateSearch);
		...
		try {
			...
			String params = "day_id='" + rideData.getDay() + "'&&time='" + timeSearch
					+ "'&&loc='" + rideData.getLocation() + "'&&" + "email='" + name
					+ "'";
		...
		if (result != null) {
			...
			try {
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					carOwner.setFirstName(jsonObject.getString(NAME));
					carOwner.setPhoneNumber(jsonObject.getString(PHONE_NUMBER));
					carOwner.setEmail(jsonObject.getString(EMAIL));
					carOwner.setStartTime(jsonObject.getString(START_TIME));
					carOwner.setEndTime(jsonObject.getString(END_TIME));
					HashMap<String, String> map = new HashMap<String, String>();

					map.put(NAME, carOwner.getFirstName());
					map.put(PHONE_NUMBER, carOwner.getPhoneNumber());
					map.put(EMAIL, carOwner.getEmail());
					map.put(START_TIME, carOwner.getStartTime());
					map.put(END_TIME, carOwner.getEndTime());
					arrayList.add(map);
				}
				/* Retrieve a List View to set the list of available Rides */
				resultView = (ListView) findViewById(R.id.listAvailable);
				ListAdapter adapter = new SimpleAdapter(
						LoadAvailableList.this,arrayList ,
	...
	private AdapterView.OnItemClickListener onListClick=new AdapterView.OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
				long id) {
			Intent i=new Intent(LoadAvailableList.this,SelectedUserDetails.class);
